...
/**
 * This class hosts the data of countries and contients. It also hosts the
 * method definitions with which any map related operations are to be performed
 * in the game.
 *
 */
public class MapGenerator {
	...
	/**
	 * Class constructor that initializes the ubiquitious countryHashMap and
	 * ContinentHashMap
	 *
	 */
	public MapGenerator() {
	...
	/**
	 * This method sets the default initial parameters according to the conquest
	 * map format.
	 *
	 */
	 ...
	 *
	 * @param inputReader
	 * @return object of buffered reader to continue reading after continents
	 *         are read.
	 * @throws IOException
	 */
	 ...
	 *
	 * @param countryname
	 * @return null if the country does not exist and the country object if the
			...
			while ((inputLine = inputReader.readLine()) != null) {
				...
				if (lineCounter == 6) {
					if (inputLine.equals("[Continents]")) {
						returnString = this.readContinentList(inputReader);// reader
																			// is
																			// returned
																			// as
																			// we
	...
	public String writeConquestFile(String fileName) {
		...
		try {
			writer = new BufferedWriter(new OutputStreamWriter(new FileOutputStream(fileName + ".map"), "utf-8"));
			writer.write("[map]");
			...
			((BufferedWriter) writer).newLine();
			for (GameContinent continent : continentHashMap.values()) { // check
																		// if
																		// for
																		// each
																		// works
	...
	/**
	 * Method builds a graph from the list of continents and country data
	 * acquired.
	 *
	...
	/**
	 * This method can be used to access the object of GraphUtil(The utility
	 * used for building a graph)
	 *
	 * @return
	 */
	...
	public String addCountry(String continentname, String countryName, ArrayList<String> neighbours) {
		...
		try {
			...
			if ((firstCountryFlag || (!countryHashMap.containsKey(countryName))
					|| ((countryHashMap.containsKey(countryName))
							&& countryHashMap.get(countryName).getContinent() == null))) {
	...
	/**
	 * This method validates a map when the user wants to load the map after
	 * reading , editing a map.
	 *
	...
	public String addNeighbor(String countryName, String neighborName) {
		try {
			 ...
			 * countryHashMap.get(neighborName).setNeighbouringCountries(
			 * neighbor); }
			 */
			return "SUCCESS";
		} catch (NullPointerException e) {
	...
	public String removeCountry(String countryName) {
		try {
			String returnString;
			if (countryHashMap.containsKey(countryName)) {
				countryHashMap.get(countryName).getContinent(). // Removes the
																// country from
																// its
																// continent's
																// countrylist.
	...
	/**
	 * This method removes a continent from database when all countries from it
